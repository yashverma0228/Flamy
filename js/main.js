...
const Flamy_config = {
    ...
    // Cursor Following
    cursorFollow: '.Flamy-socials-list a, .Flamy-gallery-nav',
...
jQuery(function ($) {
    ...
    const core = new ST_Core(Flamy_config);
    ...
    function MainMenuInit() {
        template.menu = new Flamy_Menu(core);
    }

    // --- PHOTOSWITE LIGHTBOX --- //
    function initPSWP() {
        if (jQuery('.Flamy-lightbox-link').length) {
            if (typeof PhotoSwipe === 'function') {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close Flamy-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button Flamy-pswp-nav Flamy-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button Flamy-pswp-nav Flamy-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
                ...
                // Build PSWP Gallery
                jQuery('.Flamy-lightbox-link').each(function () {
                    ...
                    if ($this.attr('data-type') && $this.attr('data-type') !== 'image') {
                        ...
                        if ('video' === $this.attr('data-type')) {
                            this_item.html = '\
                            <div class="Flamy-pswp-media--video" data-src="' + $this.attr('href') + '">\
                            ...
                            this_item.html = '\
                            <div class="Flamy-pswp-media--iframe" data-player-src="'+ this_link + '" data-src="' + $this.attr('href') + '" data-type="' + this_type + '">\
                ...
                core.$dom.body.on('click', '.Flamy-pswp-image-wrap', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                ...
                jQuery(document).on('click', '.Flamy-lightbox-link', function (e) {
                    e.preventDefault();
                    if (typeof PhotoSwipe === 'function') {
                        let $this = jQuery(this),
                        ...
                        template.pswp.lightbox.listen('resize', function () {
                            if (jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--video').length) {
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                            if (jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--iframe').length) {
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                        });
                        ...
                        template.pswp.lightbox.listen('beforeChange', function () {
                            ...
                            if (jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--video').length) {
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--video').each(function () {
                            ...
                            if (jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--iframe').length) {
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.Flamy-pswp-media--iframe').each(function () {
                ...
                console.error('Flamy Error: PSWP script not found.');
            }
        }
    }
    ...
    function HeroSections() {
        // Masked Blocks
        if (jQuery('.Flamy-masked-block').length) {
            jQuery('.Flamy-masked-block').each(function () {
                new Flamy_Masked(this);
            });
        }
    }
    ...
    function BlocksHighlight() {
        if (jQuery('.Flamy-backlight').length) {
            template.cursor_pos = {
                x: 0,
                y: 0
            }
            jQuery(document).on('mousemove', function (e) {
                ...
                jQuery('.Flamy-backlight').css({
                    '--mouse-x': `${template.cursor_pos.x}px`,
                    '--mouse-y': `${template.cursor_pos.y}px`,
                });
            });

            if (jQuery('.Flamy-block').length) {
                jQuery('.Flamy-block').each(function () {
    ...
    // --- MARQUEE ELEMENT --- //
    function Marquee() {
        if (jQuery('.Flamy-marquee').length) {
            jQuery('.Flamy-marquee').each(function () {
                new Flamy_Marquee(this);
            });
        }
    }
        ...
        if (jQuery('.swiper').length) {
            ...
            const init_swiper = function () {
                ...
                jQuery('.swiper').each(function () {
                    ...
                    if (core.$dom.body.hasClass('Flamy-fullscreen-page')) {
                        options.mousewheel = true;
                    }
                    // CAROUSEL
                    if ($this.hasClass('Flamy-carousel')) {
                    ...
                    if ($this.hasClass('Flamy-slider')) {
                        ...
                        // Slider Content
                        if ($this.find('.Flamy-slide-content').length) {
                            ...
                            $this.children('.swiper-wrapper').children('.swiper-slide').each(function (i) {
                                ...
                                if ($slide.children('.Flamy-slide-content').length) {
                                    const $slide_content = $slide.children('.Flamy-slide-content');
                                    ...
                                    $media.wrap('<div class="Flamy-slider-media"/>');
                                    ...
                                    $slide.children('.Flamy-slider-media').css('clip-path', `url(#${(content_mask_id + i)})`);
                    ...
                    // Pagination
                    if ($this.children('.Flamy-dots').length) {
                        options.pagination = {
                            el: '.Flamy-dots',
                            dynamicBullets: true,
                    ...
                    // Slider Navigation
                    if ($this.parent().children('.Flamy-slider-nav').length) {
                        let $this_nav = $this.parent().children('.Flamy-slider-nav');
                        options.navigation = {
                            prevEl: $this_nav.children('.Flamy-slider-prev')[0],
                            nextEl: $this_nav.children('.Flamy-slider-next')[0],
                        }
            ...
            if (typeof Swiper === 'function') {
                ...
                console.error('Flamy Error: Swiper Library not found.')
            }
        }
    }
    ...
    function CircleLinks() {
        if (jQuery('.Flamy-circle-path-link:not(.is-init)').length) {
            jQuery('.Flamy-circle-path-link:not(.is-init)').each(function () {
                let $this = jQuery(this),
    ...
    function LinksEvents() {
        jQuery('.Flamy-back-to-listing').on('click', function () {
            window.localStorage.setItem('Flamy_return_from', this.getAttribute('data-id'));
        });
        if (jQuery('.Flamy-albums-listing').length && window.localStorage.getItem('Flamy_return_from') != false) {
            jQuery(window).trigger('Flamy_set_album', window.localStorage.getItem('Flamy_return_from'));
            window.localStorage.setItem('Flamy_return_from', 0);
        } else {
            window.localStorage.setItem('Flamy_return_from', 0);
        }
    }
    ...
    function InfiniteList() {
        if (jQuery('.Flamy-infinite-list-wrap:not(.is-init)').length) {
            jQuery('.Flamy-infinite-list-wrap:not(.is-init)').each(function () {
                this.classList.add('is-init');
                core.updateRequired.push(new Flamy_Infinite_List(this));
            })
        }
    }
    ...
    // --- COUNTERS --- //
    function Counters() {
        if (jQuery('.Flamy-counter:not(.is-init)').length) {
            jQuery('.Flamy-counter:not(.is-init)').each(function () {
                let $this = jQuery(this);
                this.classList.add('is-init');
                this.counter_count = function () {
                    let $counter = $this.children('.Flamy-counter-number');
    ...
    function ListWithPreview() {
        if (jQuery('.Flamy-list-with-preview').length) {
            jQuery('.Flamy-list-with-preview').each(function () {
                new Flamy_LWP(this);
            });
        }
    }
    ...
    function BeforeAfter() {
        if (jQuery('.Flamy-before-after:not(.is-init)').length) {
            jQuery('.Flamy-before-after:not(.is-init)').each(function () {
                jQuery(this).addClass('is-init');
                core.updateRequired.push(new Flamy_Before_After(jQuery(this)));
            });
        }
    }
    ...
    // --- TOGGLES --- //
    function Toggles() {
        if (jQuery('.Flamy-toggles-item:not(.is-init)').length) {
            jQuery('.Flamy-toggles-item:not(.is-init)').each(function () {
                let $this = jQuery(this);
                $this.addClass('is-init');
                $this.children('.Flamy-toggles-item--title').append('<i class="Flamy-icon Flamy-icon-arrow-down"/>');
                $this.children('.Flamy-toggles-item--content').slideUp(1);
                $this.on('click', function () {
                    jQuery(this).toggleClass('is-active');
                    $this.children('.Flamy-toggles-item--content').slideToggle(300);
                });
                $this.on('click', '.Flamy-toggles-item--content', function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                });
            });
    ...
    function ScrollTrigger() {
        ...
        if (jQuery('.Flamy-inline-ribbon:not(.is-init)').length) {
            jQuery('.Flamy-inline-ribbon:not(.is-init)').each(function () {
                if (core.sap !== null) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Sticky Elements
        if (jQuery('.Flamy-sticky-block:not(.is-init)').length) {
            jQuery('.Flamy-sticky-block:not(.is-init)').each(function () {
                if (core.sap !== null) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Expand on Scroll
        if (jQuery('.Flamy-expand-on-scroll:not(.is-init)').length) {
            jQuery('.Flamy-expand-on-scroll:not(.is-init)').each(function () {
                if (core.sap !== null) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Text Mask Fill
        if (jQuery('.Flamy-text-fill-mask:not(.is-init)').length) {
            jQuery('.Flamy-text-fill-mask:not(.is-init)').each(function () {
                if (core.sap !== null) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Parallax Media
        if (jQuery('.Flamy-parallax-media:not(.is-init)').length) {
            jQuery('.Flamy-parallax-media:not(.is-init)').each(function () {
                let $this = jQuery(this);
    ...
    function ContactForm() {
        if (jQuery('.Flamy-contact-form').length) {
            // The code related to the contact form is in the file: js/contact_form.js
            if (typeof Flamy_Contact_Form === 'function') {
                Flamy_Contact_Form();
            } else {
                console.error('Flamy Error: Contact form script not found.')
            }
        }
    }
    ...
    function InitAnimation() {
        ...
        template.checkURL = function (el) {
            ...
            if (this_href.indexOf('javascript') === 0) {
                ...
                jQuery(Flamy_config.linksException).each(function () {
                    if ($el.is(this)) {
                        result = false;
                    }
                });
    ...
    function Layout() {
        ...
        if (core.$dom.body.hasClass('Flamy-fullscreen-page')) {
            ...
            core.$dom.main.removeClass('Flamy-min-content');
            ...
            if (core.$dom.main.height() < minHeight) {
                core.$dom.main.addClass('Flamy-min-content');
    ...
    $(document).ready(function () {
        // General UI
        RightClickProtection('.Flamy-rcp-wrap');
        MainMenuInit();
        CircleLinks();
